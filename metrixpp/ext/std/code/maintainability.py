...
@package OmanMaintainability
Oman's maintainability metrics:
...
class Plugin(api.Plugin,
    ...
    def declare_configuration(self, parser):
        self.parser = parser
        parser.add_option("--std.code.oman.all", "--scoa", action="store_true", default=False,
            help="Maintainability metrics plugin; the following modules are required: "
                 "--std.code.lines.comments, "
                 "--std.code.lines.total, "
                 "--std.code.complexity.cyclomatic, "
                 "and either --std.code.halstead.all "
                 "or --std.code.halstead.base and --std.code.halstead.volume "
                 "[default: %default]")

    def configure(self, options):
        self.is_active_oman = options.__dict__['std.code.oman.all']
        if self.is_active_oman == True:
            ...
            for each in required_opts:
                if options.__dict__[each] == False:
                    self.parser.error('option --std.code.oman.all: requires --{0} option'.
                                      format(each))
            req_or = False
            required_opts = ['std.code.halstead.all',
            ...
            for each in required_opts:
                if options.__dict__[each] == True:
                    ...
                    break
            if ( req_or == False ):
                self.parser.error('option --std.code.oman.all: requires --std.code.halstead.all or --std.code.halstead.volume option')
...
#            print("Hello world")
    ...
    class OmanCalculator(api.MetricPluginMixin.PlainCounter):
        ...
        def get_OmanFields(self):
            ...
            self.aveV = self.region.get_data('std.code.halstead_adv', 'H_Volume')
            self.aveLOC = self.LOCphy
            #print(self.region.name+': '+str(self.LOCcom)+"|"+str(self.LOCphy)+"|"+str(self.aveG)+"|"+str(self.aveV))
    ...
    class Oman_perCM(OmanCalculator):
        """ average percent of lines of comments per Module
            Note that this metric is saved as percentage
        """
        def get_result(self):
